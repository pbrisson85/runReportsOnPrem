...
const getWeeksMap = async () => {
  ...
  const map = await sql`
      SELECT w.week_serial, w.fiscal_year, w.week, w.period_serial, w.period_num, w.date_start, w.date_end, w.date_start || ' (' || RIGHT(w.week_serial,3) || ') ' AS display_start, w.date_end || ' (' || RIGHT(w.week_serial,3) || ') ' AS display_end, 'weeks' AS map
