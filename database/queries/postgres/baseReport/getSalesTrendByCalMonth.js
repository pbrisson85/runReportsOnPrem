...
const l5_getSalesByCalMo = async (config, start, end) => {
  try {
    ...
    const response = await sql
      ...
      GROUP BY EXTRACT('MONTH' FROM sales_line_items.formatted_invoice_date), ${sql(config.l1_field)}, ${sql(config.l2_field)}, ${sql(config.l3_field)}, ${sql(config.l4_field)}, ${sql(config.l5_field)} 
...
module.exports.l5_getSalesCalMoToDate = l5_getSalesCalMoToDate
