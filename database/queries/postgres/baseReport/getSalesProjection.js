...
const l1_getSalesProjectionByWk = async (config, start, end) => {
  try {
    ...
    const response = await sql
      `SELECT pj.column, pj.l1_label, 'SUBTOTAL' AS l2_label, 'SUBTOTAL' AS l3_label, 'SUBTOTAL' AS l4_label, SUM(pj.lbs) AS lbs, SUM(pj.sales) AS sales, SUM(pj.cogs) AS cogs, SUM(pj.othp) AS othp 
      ...
      FROM (
        ...
        UNION 
          SELECT so.week_serial column, COALESCE(${sql(config.l1_field)},'BLANK') AS l1_label, COALESCE(so.ext_weight,0) AS lbs, COALESCE(so.ext_sales,0) AS sales, COALESCE(so.ext_cost,0) AS cogs, COALESCE(so.ext_othp,0) AS othp 
...
const l2_getSalesProjectionByWk = async (config, start, end) => {
  try {
    ...
    const response = await sql
      `SELECT pj.column, pj.l1_label, pj.l2_label, 'SUBTOTAL' AS l3_label, 'SUBTOTAL' AS l4_label, SUM(pj.lbs) AS lbs, SUM(pj.sales) AS sales, SUM(pj.cogs) AS cogs, SUM(pj.othp) AS othp 
      ...
      FROM (
        ...
        UNION 
          SELECT so.week_serial column, COALESCE(${sql(config.l1_field)},'BLANK') AS l1_label, COALESCE(${sql(config.l2_field)},'NA') AS l2_label, COALESCE(so.ext_weight,0) AS lbs, COALESCE(so.ext_sales,0) AS sales, COALESCE(so.ext_cost,0) AS cogs, COALESCE(so.ext_othp,0) AS othp 
...
const l3_getSalesProjectionByWk = async (config, start, end) => {
  try {
    ...
    const response = await sql
      `SELECT pj.column, pj.l1_label, pj.l2_label, pj.l3_label, 'SUBTOTAL' AS l4_label, SUM(pj.lbs) AS lbs, SUM(pj.sales) AS sales, SUM(pj.cogs) AS cogs, SUM(pj.othp) AS othp 
      ...
      FROM (
        ...
        UNION 
          SELECT so.week_serial column, COALESCE(${sql(config.l1_field)},'BLANK') AS l1_label, COALESCE(${sql(config.l2_field)},'NA') AS l2_label, COALESCE(${sql(config.l3_field)},'NA') AS l3_label, COALESCE(so.ext_weight,0) AS lbs, COALESCE(so.ext_sales,0) AS sales, COALESCE(so.ext_cost,0) AS cogs, COALESCE(so.ext_othp,0) AS othp 
...
const l4_getSalesProjectionByWk = async (config, start, end) => {
  try {
    ...
    const response = await sql
      `SELECT pj.column, pj.l1_label, pj.l2_label, pj.l3_label, pj.l4_label, SUM(pj.lbs) AS lbs, SUM(pj.sales) AS sales, SUM(pj.cogs) AS cogs, SUM(pj.othp) AS othp 
...
const l0_getSalesProjectionByWk = async (config, start, end) => {
  try {
    ...
    const response = await sql
      `
      SELECT pj.column, pj.l1_label, 'TOTAL' AS l2_label, 'TOTAL' AS l3_label, 'TOTAL' AS l4_label, SUM(pj.lbs) AS lbs, SUM(pj.sales) AS sales, SUM(pj.cogs) AS cogs, SUM(pj.othp) AS othp 
