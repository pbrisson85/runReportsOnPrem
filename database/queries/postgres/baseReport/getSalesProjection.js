...
const l1_getSalesProjectionByWk = async (config, start, end) => {
  try {
    ...
    const response = await sql
      ...
      FROM (
        SELECT sl.week_serial AS column, COALESCE(sl.calc_gm_rept_weight,0) AS lbs, COALESCE(sl.gross_sales_ext,0) AS sales, COALESCE(sl.cogs_ext_gl,0) AS cogs, COALESCE(sl.othp_ext,0) AS othp 
        ...
        UNION 
          SELECT so.week_serial AS column, COALESCE(so.ext_weight,0) AS lbs, COALESCE(so.ext_sales,0) AS sales, COALESCE(so.ext_cost,0) AS cogs, COALESCE(so.ext_othp,0) AS othp 
