...
const getFgSalesOrdersByProgram = async () => {
  try {
    ...
    const response = await pgClient.query(
         'SELECT \'FG OPEN ORDER\' AS column, master_supplement.species_group AS l1_grouping, master_supplement.program AS l2_grouping, SUM(sales_orders.ext_weight) AS lbs, SUM(sales_orders.ext_sales) AS sales, SUM(sales_orders.ext_cost) AS cogs, SUM(sales_orders.ext_othp) AS othp FROM "salesReporting".sales_orders LEFT OUTER JOIN "invenReporting".master_supplement ON master_supplement.item_num = sales_orders.item_num WHERE master_supplement.item_type = $1 AND sales_orders.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) GROUP BY master_supplement.species_group, master_supplement.program', ['FG']
        ) //prettier-ignore
...
const getFgSalesOrdersBySpecies = async () => {
  try {
    ...
    const response = await pgClient.query(
           'SELECT \'FG OPEN ORDER\' AS column, master_supplement.species_group AS l1_grouping, \'SUBTOTAL\' AS l2_grouping, SUM(sales_orders.ext_weight) AS lbs, SUM(sales_orders.ext_sales) AS sales, SUM(sales_orders.ext_cost) AS cogs, SUM(sales_orders.ext_othp) AS othp FROM "salesReporting".sales_orders LEFT OUTER JOIN "invenReporting".master_supplement ON master_supplement.item_num = sales_orders.item_num WHERE master_supplement.item_type = $1 AND sales_orders.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) GROUP BY master_supplement.species_group', ['FG']
          ) //prettier-ignore
...
const getFgSalesOrdersTotal = async () => {
  try {
    ...
    const response = await pgClient.query(
           'SELECT \'FG OPEN ORDER\' AS column, \'FG SALES\' AS l1_grouping, \'TOTAL\' AS l2_grouping, SUM(sales_orders.ext_weight) AS lbs, SUM(sales_orders.ext_sales) AS sales, SUM(sales_orders.ext_cost) AS cogs, SUM(sales_orders.ext_othp) AS othp FROM "salesReporting".sales_orders LEFT OUTER JOIN "invenReporting".master_supplement ON master_supplement.item_num = sales_orders.item_num WHERE master_supplement.item_type = $1 AND sales_orders.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders)', ['FG']
          ) //prettier-ignore
