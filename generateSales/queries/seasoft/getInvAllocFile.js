...
const getInvAllocFile = async (firstDayOfFy, firstDayOfNextFy) => {
  try {
    ...
    const queryString = "SELECT \"Invoice Allocations\".INVOICE_DATE, \"Invoice Allocations\".INVOICE_NUMBER, \"Invoice Allocations\".INVOICE_LINE_NUMBER, \"Invoice Allocations\".PRODUCT_NUMBER, \"Invoice Allocations\".EXPENSE_AMOUNT, \"Invoice Allocations\".EXPENSE_CODE, \"General Table File\".TABLE_FLD02 FROM {OJ 'Invoice Allocations' LEFT OUTER JOIN 'General Table File' ON \"Invoice Allocations\".EXPENSE_CODE = \"General Table File\".TABLE_CODE} WHERE \"Invoice Allocations\".EXPENSE_CODE <> 'SALE' AND \"General Table File\".TABLE_ID = 'OTHP' AND \"Invoice Allocations\".INVOICE_DATE >= ? AND \"Invoice Allocations\".INVOICE_DATE < ?" //prettier-ignore
