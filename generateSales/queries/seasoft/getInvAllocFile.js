...
const getInvAllocFile = async (firstDayOfFy, firstDayOfNextFy) => {
  try {
    ...
    const queryString = "SELECT \"Invoice Allocations\".INVOICE_DATE, {fn RTRIM(\"Invoice Allocations\".INVOICE_NUMBER)}, {fn RTRIM(\"Invoice Allocations\".INVOICE_LINE_NUMBER)}, {fn RTRIM(\"Invoice Allocations\".PRODUCT_NUMBER)}, \"Invoice Allocations\".EXPENSE_AMOUNT, {fn RTRIM(\"Invoice Allocations\".EXPENSE_CODE}) FROM 'Invoice Allocations' WHERE \"Invoice Allocations\".EXPENSE_CODE <> 'SALE' AND \"Invoice Allocations\".INVOICE_DATE >= ? AND \"Invoice Allocations\".INVOICE_DATE < ?" //prettier-ignore
