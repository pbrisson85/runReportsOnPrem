    ...
    const response = await sql
        ...
        UNION SELECT ms.item_num AS l1_label, ms.description AS l2_label, ${sql(config.l2_field)} AS l3_label , ${sql(config.l3_field)} AS l4_label 
          ...
          ON ms.item_num = sales_orders.item_num 
            ...
            WHERE ms.byproduct_type IS NULL AND ms.item_type = ${'FG'} AND ms.program = ${program} AND sales_orders.version = (SELECT MAX(sales_orders.version) - 1 FROM "salesReporting".sales_orders) AND ${sql(config.l1_field)} = ${filters[0]} AND sales_orders.customer_code = ${filters[3]}
