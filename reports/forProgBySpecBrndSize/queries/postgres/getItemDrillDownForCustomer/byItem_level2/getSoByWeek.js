...
const lvl_1_subtotal_getSo_byWk = async (config, program, filters) => {
  try {
    ...
    const response = await sql
         ...
         GROUP BY sales_orders.week_serial, ms.item_num, ms.description, ${sql(config.l3_field)} 
...
const lvl_0_total_getSo_byWk = async (config, program, filters) => {
  try {
    ...
    const response = await sql
           ...
           FROM "salesReporting".sales_orders) AND ms.program = ${program} AND ms.byproduct_type IS NULL AND ${sql(config.l1_field)} = ${filters[0]} AND ${sql(config.l2_field)} = ${filters[1]} AND sales_orders.customer_code = ${filters[3]} 
