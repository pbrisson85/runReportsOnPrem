...
const lvl_1_subtotal_getSo = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
         ...
         WHERE ms.item_type = $1 AND sales_orders.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.program = $2 AND ms.byproduct_type IS NULL AND ms.species = $3 AND ms.brand = $4 
         
         GROUP BY ms.item_num, ms.description, ms.size_name`, ['FG', program, filters[0], filters[1]]
        ) //prettier-ignore
...
const lvl_1_subtotal_getSoTagged = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
           ...
           WHERE ms.item_type = $1 AND sales_orders.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.program = $2 AND ms.byproduct_type IS NULL AND sales_orders.tagged_weight > 0 AND ms.species = $3 AND ms.brand = $4 
...
const lvl_1_subtotal_getSoUntagged = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.item_type = $1 AND sales_orders.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.program = $2 AND ms.byproduct_type IS NULL AND sales_orders.untagged_weight > 0 AND ms.species = $3 AND ms.brand = $4 
...
const lvl_0_total_getSo = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
           ...
           WHERE ms.item_type = $1 AND sales_orders.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.program = $2 AND ms.byproduct_type IS NULL AND ms.species = $3 AND ms.brand = $4`, ['FG', program, filters[0], filters[1]]
          ) //prettier-ignore
...
const lvl_0_total_getSoTagged = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.item_type = $1 AND sales_orders.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.program = $2 AND ms.byproduct_type IS NULL AND sales_orders.tagged_weight > 0 AND ms.species = $3 AND ms.brand = $4`, ['FG', program, filters[0], filters[1]]
          ) //prettier-ignore
...
const lvl_0_total_getSoUntagged = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.item_type = $1 AND sales_orders.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.program = $2 AND ms.byproduct_type IS NULL AND sales_orders.untagged_weight > 0 AND ms.species = $3 AND ms.brand = $4`, ['FG', program, filters[0], filters[1]]
          ) //prettier-ignore
