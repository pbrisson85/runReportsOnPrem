...
const lvl_1_subtotal_getSo_byWk = async (config, program, filters) => {
  try {
    ...
    GROUP BY sales_orders.week_serial, ms.item_num, ms.description, ms.size_name ORDER BY sales_orders.week_serial` //prettier-ignore
...
const lvl_0_total_getSo_byWk = async (config, program, filters) => {
  try {
    ...
    const response = await sql
           ...
           GROUP BY sales_orders.week_serial ORDER BY sales_orders.week_serial` //prettier-ignore
