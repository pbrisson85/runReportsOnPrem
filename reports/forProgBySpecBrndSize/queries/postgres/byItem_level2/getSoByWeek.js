...
const lvl_1_subtotal_getSo_byWk = async (program, filters) => {
  try {
    ...
    GROUP BY sales_orders.week_serial, ms.item_num, ms.description, ms.size_name ORDER BY sales_orders.week_serial`, ['FG', program, filters[0], filters[1]]
  ) //prettier-ignore
...
const lvl_0_total_getSo_byWk = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
           ...
           GROUP BY sales_orders.week_serial ORDER BY sales_orders.week_serial`, ['FG', program, filters[0], filters[1]]
          ) //prettier-ignore
...
const lvl_0_total_getSoUntagged_byWk = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      ORDER BY sales_orders.week_serial`, ['FG', program, filters[0], filters[1]]
          ) //prettier-ignore
