  ...
  try {
    ...
    const response = await pgClient.query(
        ...
        UNION SELECT so.customer_code AS l1_label, so.customer_name AS l2_label 
          ...
          WHERE ms.byproduct_type IS NULL AND ms.item_type = $3 AND ms.program = $4 AND so.version = (SELECT MAX(sales_orders.version) - 1 FROM "salesReporting".sales_orders) AND ms.species = $5 
