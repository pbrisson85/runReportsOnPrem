...
const lvl_1_subtotal_getFgInven = async program => {
  try {
    ...
    const response = await pgClient.query(
      `SELECT 'FG INVEN' AS column, ms.species AS l1_label, 'SUBTOTAL' AS l2_label, 'SUBTOTAL' AS l3_label, COALESCE(SUM(perpetual_inventory.on_hand_lbs),0) AS lbs, COALESCE(SUM(perpetual_inventory.cost_extended),0) AS cogs 
...
const lvl_1_subtotal_getFgInTransit = async program => {
  try {
    ...
    const response = await pgClient.query(
      `SELECT 'FG IN TRANSIT' AS column, ms.species AS l1_label, 'SUBTOTAL' AS l2_label, 'SUBTOTAL' AS l3_label, COALESCE(SUM(perpetual_inventory.on_hand_lbs),0) AS lbs, COALESCE(SUM(perpetual_inventory.cost_extended),0) AS cogs 
      
      FROM "invenReporting".perpetual_inventory LEFT OUTER JOIN "invenReporting".master_supplement AS ms ON ms.item_num = perpetual_inventory.item_number 
      
      WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND perpetual_inventory.version = (SELECT MAX(perpetual_inventory.version) - 1 FROM "invenReporting".perpetual_inventory) AND perpetual_inventory.location_type = $2 AND ms.program = $3 
...
const lvl_1_subtotal_getFgAtLoc = async program => {
  try {
    ...
    const response = await pgClient.query(
      `SELECT 'FG ON HAND' AS column, ms.species AS l1_label, 'SUBTOTAL' AS l2_label, 'SUBTOTAL' AS l3_label, COALESCE(SUM(perpetual_inventory.on_hand_lbs),0) AS lbs, COALESCE(SUM(perpetual_inventory.cost_extended),0) AS cogs 
...
const lvl_1_subtotal_getFgAtLoc_untagged = async program => {
  try {
    ...
    const response = await pgClient.query(
 `SELECT 'FG ON HAND UNTAGGED' AS column, ms.species AS l1_label, 'SUBTOTAL' AS l2_label, 'SUBTOTAL' AS l3_label, COALESCE(SUM(inven_t.on_hand_lbs),0) - COALESCE(SUM(tagged_t.weight),0) AS lbs , COALESCE(SUM(inven_t.cost_extended),0) - COALESCE(SUM(tagged_t.ext_cost),0) AS cogs 
 ...
 FROM (
  SELECT pi.cost_extended, pi.item_number, pi.lot, pi.on_hand_lbs, pi.location_code 
    ...
    WHERE pi .version = (SELECT MAX(subpi.version) - 1 FROM "invenReporting".perpetual_inventory AS subpi) AND pi.location_type <> $2) AS inven_t 
  ...
  LEFT OUTER JOIN (
    ...
    WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND ms.program = $3 
    
    GROUP BY ms.species`, ['FG', 'IN TRANSIT', program]
    ) //prettier-ignore
...
const lvl_1_subtotal_getFgAtLoc_tagged = async program => {
  try {
    ...
    const response = await pgClient.query(
      `SELECT 'FG ON HAND TAGGED' AS column, ms.species AS l1_label, 'SUBTOTAL' AS l2_label, 'SUBTOTAL' AS l3_label, COALESCE(SUM(tagged_inventory.weight),0) AS lbs, COALESCE(SUM(tagged_inventory.cost * tagged_inventory.weight),0) AS cogs 
      ...
      WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND tagged_inventory.version = (SELECT MAX(tagged_inventory.version) - 1 FROM "salesReporting".tagged_inventory) AND ms.program = $2 
...
const lvl_2_subtotal_getFgInven = async program => {
  try {
    ...
    const response = await pgClient.query(
      `SELECT 'FG INVEN' AS column, ms.species AS l1_label, ms.brand AS l2_label, 'SUBTOTAL' AS l3_label, COALESCE(SUM(perpetual_inventory.on_hand_lbs),0) AS lbs, COALESCE(SUM(perpetual_inventory.cost_extended),0) AS cogs 
...
const lvl_2_subtotal_getFgInTransit = async program => {
  try {
    ...
    const response = await pgClient.query(
      `SELECT 'FG IN TRANSIT' AS column, ms.species AS l1_label, ms.brand AS l2_label, 'SUBTOTAL' AS l3_label, COALESCE(SUM(perpetual_inventory.on_hand_lbs),0) AS lbs, COALESCE(SUM(perpetual_inventory.cost_extended),0) AS cogs 
...
const lvl_2_subtotal_getFgAtLoc = async program => {
  try {
    ...
    const response = await pgClient.query(
      `SELECT 'FG ON HAND' AS column, ms.species AS l1_label, ms.brand AS l2_label, 'SUBTOTAL' AS l3_label, COALESCE(SUM(perpetual_inventory.on_hand_lbs),0) AS lbs, COALESCE(SUM(perpetual_inventory.cost_extended),0) AS cogs 
...
const lvl_2_subtotal_getFgAtLoc_untagged = async program => {
  try {
    ...
    const response = await pgClient.query(
 `SELECT 'FG ON HAND UNTAGGED' AS column, ms.species AS l1_label, ms.brand AS l2_label, 'SUBTOTAL' AS l3_label, COALESCE(SUM(inven_t.on_hand_lbs),0) - COALESCE(SUM(tagged_t.weight),0) AS lbs , COALESCE(SUM(inven_t.cost_extended),0) - COALESCE(SUM(tagged_t.ext_cost),0) AS cogs 
 ...
 FROM (
  ...
  GROUP BY ms.species, ms.brand`, ['FG', 'IN TRANSIT', program]
    ) //prettier-ignore
...
const lvl_2_subtotal_getFgAtLoc_tagged = async program => {
  try {
    ...
    const response = await pgClient.query(
      `SELECT 'FG ON HAND TAGGED' AS column, ms.species AS l1_label, ms.brand AS l2_label, 'SUBTOTAL' AS l3_label, COALESCE(SUM(tagged_inventory.weight),0) AS lbs, COALESCE(SUM(tagged_inventory.cost * tagged_inventory.weight),0) AS cogs 
...
const lvl_3_subtotal_getFgInven = async program => {
  try {
    ...
    const response = await pgClient.query(
      `SELECT 'FG INVEN' AS column, ms.species AS l1_label, ms.brand AS l2_label, ms.size_name AS l3_label, COALESCE(SUM(perpetual_inventory.on_hand_lbs),0) AS lbs, COALESCE(SUM(perpetual_inventory.cost_extended),0) AS cogs 
...
const lvl_3_subtotal_getFgInTransit = async program => {
  try {
    ...
    const response = await pgClient.query(
      `SELECT 'FG IN TRANSIT' AS column, ms.species AS l1_label, ms.brand AS l2_label, ms.size_name AS l3_label, COALESCE(SUM(perpetual_inventory.on_hand_lbs),0) AS lbs, COALESCE(SUM(perpetual_inventory.cost_extended),0) AS cogs 
...
const lvl_3_subtotal_getFgAtLoc = async program => {
  try {
    ...
    const response = await pgClient.query(
      `SELECT 'FG ON HAND' AS column, ms.species AS l1_label, ms.brand AS l2_label, ms.size_name AS l3_label, COALESCE(SUM(perpetual_inventory.on_hand_lbs),0) AS lbs, COALESCE(SUM(perpetual_inventory.cost_extended),0) AS cogs 
...
const lvl_3_subtotal_getFgAtLoc_untagged = async program => {
  try {
    ...
    const response = await pgClient.query(
 `SELECT 'FG ON HAND UNTAGGED' AS column, ms.species AS l1_label, ms.brand AS l2_label, ms.size_name AS l3_label, COALESCE(SUM(inven_t.on_hand_lbs),0) - COALESCE(SUM(tagged_t.weight),0) AS lbs , COALESCE(SUM(inven_t.cost_extended),0) - COALESCE(SUM(tagged_t.ext_cost),0) AS cogs 
 
 FROM (
      SELECT pi.cost_extended, pi.item_number, pi.lot, pi.on_hand_lbs, pi.location_code 
      FROM "invenReporting".perpetual_inventory AS pi 
      WHERE pi .version = (SELECT MAX(subpi.version) - 1 FROM "invenReporting".perpetual_inventory AS subpi) AND pi.location_type <> $2) 
      AS inven_t 
...
const lvl_3_subtotal_getFgAtLoc_tagged = async program => {
  try {
    ...
    const response = await pgClient.query(
      `SELECT 'FG ON HAND TAGGED' AS column, ms.species AS l1_label, ms.brand AS l2_label, ms.size_name AS l3_label, COALESCE(SUM(tagged_inventory.weight),0) AS lbs, COALESCE(SUM(tagged_inventory.cost * tagged_inventory.weight),0) AS cogs 
...
const lvl_0_total_getFgInTransit = async program => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND perpetual_inventory.version = (SELECT MAX(perpetual_inventory.version) - 1 FROM "invenReporting".perpetual_inventory) AND ms.program = $2 AND perpetual_inventory.location_type = $3`,
...
const lvl_0_total_getFgAtLoc_untagged = async program => {
 ...
 FROM (
  ...
  LEFT OUTER JOIN (
        ...
        GROUP BY ti.lot, ti.location, ti.item_num) AS tagged_t 
    ON tagged_t.item_num = inven_t.item_number AND tagged_t.lot = inven_t.lot AND tagged_t.location = inven_t.location_code 
  WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND ms.program = $3`, ['FG', 'IN TRANSIT', program]
    ) //prettier-ignore
