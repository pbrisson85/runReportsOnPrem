...
router.post('/', async (req, res) => {
  ...
  const { program, option, filters, columnDataName, colType, periodEnd, showFyTrend, format, queryLevel } = req.body
  ...
  const response = await buildDrilldown(filters[0], periodStart, periodEnd, program, filters, showFyTrend, startWeek, endWeek, queryLevel)
