...
const lvl_3_subtotal_getSo_byWk = async (config, program) => {
  try {
    ...
    const response = await sql
      `SELECT sales_orders.week_serial || '_so' AS column, ${sql(config.l1_field)} AS l1_label, COALESCE(${sql(config.l2_field)},'NA') AS l2_label, ${sql(config.l3_field)} AS l3_label, COALESCE(SUM(sales_orders.ext_weight),0) AS lbs, COALESCE(SUM(sales_orders.ext_sales),0) AS sales, COALESCE(SUM(sales_orders.ext_cost),0) AS cogs, COALESCE(SUM(sales_orders.ext_othp),0) AS othp 
