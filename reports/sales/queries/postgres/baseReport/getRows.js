...
const getRowsFourthLevelDetail = async (config, start, end, showFyTrend) => {
  ...
  try {
    ...
    const response = await sql
        `SELECT COALESCE(${sql(config.l1_field)},'BLANK') AS l1_label, COALESCE(${sql(config.l2_field)},'NA') AS l2_label, COALESCE(${sql(config.l3_field)},'NA') AS l3_label, COALESCE(${sql(config.l4_field)},'NA') AS l4_label, 4 AS datalevel ${config.itemType ? sql`, '${sql(config.itemType)}' AS itemtype` : sql``}
        ...
        UNION SELECT COALESCE(${sql(config.l1_field)},'BLANK') AS l1_label, COALESCE(${sql(config.l2_field)},'NA') AS l2_label, COALESCE(${sql(config.l3_field)},'NA') AS l3_label, COALESCE(${sql(config.l4_field)},'NA') AS l4_label, 4 AS datalevel ${config.itemType ? sql`, '${sql(config.itemType)}' AS itemtype` : sql``}  
...
const getRowsThirdLevelDetail = async (config, start, end, showFyTrend) => {
  ...
  try {
    ...
    const response = await sql
        `SELECT COALESCE(${sql(config.l1_field)},'BLANK') AS l1_label, COALESCE(${sql(config.l2_field)},'NA') AS l2_label, COALESCE(${sql(config.l3_field)},'NA') AS l3_label ${config.l4_field ? sql`, 'SUBTOTAL' AS l4_label`: sql``}, 3 AS datalevel ${config.itemType ? sql`, '${sql(config.itemType)}' AS itemtype` : sql``} 
        ...
        UNION SELECT COALESCE(${sql(config.l1_field)},'BLANK') AS l1_label, COALESCE(${sql(config.l2_field)},'NA') AS l2_label, COALESCE(${sql(config.l3_field)},'NA') AS l3_label ${config.l4_field ? sql`, 'SUBTOTAL' AS l4_label`: sql``}, 3 AS datalevel ${config.itemType ? sql`, '${sql(config.itemType)}' AS itemtype` : sql``}  
...
const getRowsSecondLevelDetail = async (config, start, end, showFyTrend) => {
  try {
    ...
    const response = await sql
        `SELECT COALESCE(${sql(config.l1_field)},'BLANK') AS l1_label, COALESCE(${sql(config.l2_field)},'NA') AS l2_label ${config.l3_field ? sql`, 'SUBTOTAL' AS l3_label`: sql``} ${config.l4_field ? sql`, 'SUBTOTAL' AS l4_label`: sql``}, 2 AS datalevel ${config.itemType ? sql`, '${sql(config.itemType)}' AS itemtype` : sql``}
        ...
        UNION SELECT COALESCE(${sql(config.l1_field)},'BLANK') AS l1_label, COALESCE(${sql(config.l2_field)},'NA') AS l2_label ${config.l3_field ? sql`, 'SUBTOTAL' AS l3_label`: sql``} ${config.l4_field ? sql`, 'SUBTOTAL' AS l4_label`: sql``}, 2 AS datalevel ${config.itemType ? sql`, '${sql(config.itemType)}' AS itemtype` : sql``} 
...
const getRowsFirstLevelDetail = async (config, start, end, showFyTrend) => {
  try {
    ...
    const response = await sql
        `SELECT COALESCE(${sql(config.l1_field)},'BLANK') AS l1_label, 'SUBTOTAL' AS l2_label ${config.l3_field ? sql`, 'SUBTOTAL' AS l3_label`: sql``} ${config.l4_field ? sql`, 'SUBTOTAL' AS l4_label`: sql``}, 1 AS datalevel ${config.itemType ? sql`, '${sql(config.itemType)}' AS itemtype` : sql``} 
        ...
        UNION SELECT COALESCE(${sql(config.l1_field)},'BLANK') AS l1_label, 'SUBTOTAL' AS l2_label ${config.l3_field ? sql`, 'SUBTOTAL' AS l3_label`: sql``} ${config.l4_field ? sql`, 'SUBTOTAL' AS l4_label`: sql``}, 1 AS datalevel ${config.itemType ? sql`, '${sql(config.itemType)}' AS itemtype` : sql``}  
