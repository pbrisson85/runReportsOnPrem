...
const getRowsThirdLevelDetail = async (config, start, end, program, showFyTrend) => {
  ...
  try {
    ...
    const response = await sql
        `SELECT ${sql(config.l1_field)} AS l1_label, COALESCE(${sql(config.l2_field)},'NA') AS l2_label, ${sql(config.l3_field)} AS l3_label 
        ...
        UNION SELECT ${sql(config.l1_field)} AS l1_label, COALESCE(${sql(config.l2_field)},'NA') AS l2_label, ${sql(config.l3_field)} AS l3_label 
...
const getRowsSecondLevelDetail = async (config, start, end, program, showFyTrend) => {
  try {
    ...
    const response = await sql
        `SELECT ${sql(config.l1_field)} AS l1_label, COALESCE(${sql(config.l2_field)},'NA') AS l2_label ${config.l3_field ? sql`, 'SUBTOTAL' AS l3_label`: sql``}
        ...
        UNION SELECT ${sql(config.l1_field)} AS l1_label, COALESCE(${sql(config.l2_field)},'NA') AS l2_label ${config.l3_field ? sql`, 'SUBTOTAL' AS l3_label`: sql``} 
...
const getRowsFirstLevelDetail = async (config, start, end, program, showFyTrend) => {
  try {
    ...
    const response = await sql
        ...
        UNION SELECT ${sql(config.l1_field)} AS l1_label, 'SUBTOTAL' AS l2_label ${config.l3_field ? sql`, 'SUBTOTAL' AS l3_label`: sql``} 
