...
const l0_getSalesWkDriven = async (config, startWk, endWk, year) => {
  try {
    ...
    const response = await sql
      `SELECT 'SALES TOTAL' AS column, '${sql(config.itemType)} SALES' AS l1_label, 'TOTAL' AS l2_label, 'TOTAL' AS l3_label, 'TOTAL' AS l4_label, COALESCE(SUM(sl.calc_gm_rept_weight),0) AS lbs, COALESCE(SUM(sl.gross_sales_ext),0) AS sales, COALESCE(SUM(sl.cogs_ext_gl),0) AS cogs, COALESCE(SUM(sl.othp_ext),0) AS othp 
