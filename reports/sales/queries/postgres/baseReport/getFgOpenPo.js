...
const l0_getFgPo = async config => {
  try {
    ...
    const response = await sql
         `SELECT 'FG ON ORDER' AS column, '${sql(config.itemType)} SALES' AS l1_label, 'TOTAL' AS l2_label, 'TOTAL' AS l3_label, 'TOTAL' AS l4_label, COALESCE(SUM(inv.on_order_lbs),0) AS lbs, COALESCE(SUM(inv.on_order_extended),0) AS cogs 
