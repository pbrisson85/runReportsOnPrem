...
const lvl_1_subtotal_getFgInven = async (config, program) => {
  try {
    ...
    const response = await sql
      `SELECT 'FG INVEN' AS column, 1 AS dataLevel, COALESCE(${sql(config.l1_field)},'BLANK') AS l1_label, 'SUBTOTAL' AS l2_label, 'SUBTOTAL' AS l3_label, COALESCE(SUM(perpetual_inventory.on_hand_lbs),0) AS lbs, COALESCE(SUM(perpetual_inventory.cost_extended),0) AS cogs 
