...
const lvl_2_subtotal_getFgInTransit = async (config, program) => {
  try {
    ...
    const response = await sql
      `SELECT 'FG IN TRANSIT' AS column, ${sql(config.l1_field)} AS l1_label, COALESCE(${sql(config.l2_field)},'NA') AS l2_label, 'SUBTOTAL' AS l3_label, COALESCE(SUM(perpetual_inventory.on_hand_lbs),0) AS lbs, COALESCE(SUM(perpetual_inventory.cost_extended),0) AS cogs 
...
const lvl_2_subtotal_getFgAtLoc = async (config, program) => {
  try {
    ...
    const response = await sql
      `SELECT 'FG ON HAND' AS column, ${sql(config.l1_field)} AS l1_label, COALESCE(${sql(config.l2_field)},'NA') AS l2_label, 'SUBTOTAL' AS l3_label, COALESCE(SUM(perpetual_inventory.on_hand_lbs),0) AS lbs, COALESCE(SUM(perpetual_inventory.cost_extended),0) AS cogs 
...
const lvl_2_subtotal_getFgAtLoc_untagged = async (config, program) => {
  try {
    ...
    const response = await sql
 `SELECT 'FG ON HAND UNTAGGED' AS column, ${sql(config.l1_field)} AS l1_label, COALESCE(${sql(config.l2_field)},'NA') AS l2_label, 'SUBTOTAL' AS l3_label, COALESCE(SUM(inven_t.on_hand_lbs),0) - COALESCE(SUM(tagged_t.weight),0) AS lbs , COALESCE(SUM(inven_t.cost_extended),0) - COALESCE(SUM(tagged_t.ext_cost),0) AS cogs 
...
const lvl_2_subtotal_getFgAtLoc_tagged = async (config, program) => {
  try {
    ...
    const response = await sql
      `SELECT 'FG ON HAND TAGGED' AS column, ${sql(config.l1_field)} AS l1_label, COALESCE(${sql(config.l2_field)},'NA') AS l2_label, 'SUBTOTAL' AS l3_label, COALESCE(SUM(tagged_inventory.weight),0) AS lbs, COALESCE(SUM(tagged_inventory.cost * tagged_inventory.weight),0) AS cogs 
...
const lvl_3_subtotal_getFgInven = async (config, program) => {
  try {
    ...
    const response = await sql
      `SELECT 'FG INVEN' AS column, ${sql(config.l1_field)} AS l1_label, COALESCE(${sql(config.l2_field)},'NA') AS l2_label, ${sql(config.l3_field)} AS l3_label, COALESCE(SUM(perpetual_inventory.on_hand_lbs),0) AS lbs, COALESCE(SUM(perpetual_inventory.cost_extended),0) AS cogs 
...
const lvl_3_subtotal_getFgInTransit = async (config, program) => {
  try {
    ...
    const response = await sql
      `SELECT 'FG IN TRANSIT' AS column, ${sql(config.l1_field)} AS l1_label, COALESCE(${sql(config.l2_field)},'NA') AS l2_label, ${sql(config.l3_field)} AS l3_label, COALESCE(SUM(perpetual_inventory.on_hand_lbs),0) AS lbs, COALESCE(SUM(perpetual_inventory.cost_extended),0) AS cogs 
...
const lvl_3_subtotal_getFgAtLoc = async (config, program) => {
  try {
    ...
    const response = await sql
      `SELECT 'FG ON HAND' AS column, ${sql(config.l1_field)} AS l1_label, COALESCE(${sql(config.l2_field)},'NA') AS l2_label, ${sql(config.l3_field)} AS l3_label, COALESCE(SUM(perpetual_inventory.on_hand_lbs),0) AS lbs, COALESCE(SUM(perpetual_inventory.cost_extended),0) AS cogs 
...
const lvl_3_subtotal_getFgAtLoc_untagged = async (config, program) => {
  try {
    ...
    const response = await sql
 `SELECT 'FG ON HAND UNTAGGED' AS column, ${sql(config.l1_field)} AS l1_label, COALESCE(${sql(config.l2_field)},'NA') AS l2_label, ${sql(config.l3_field)} AS l3_label, COALESCE(SUM(inven_t.on_hand_lbs),0) - COALESCE(SUM(tagged_t.weight),0) AS lbs , COALESCE(SUM(inven_t.cost_extended),0) - COALESCE(SUM(tagged_t.ext_cost),0) AS cogs 
...
const lvl_3_subtotal_getFgAtLoc_tagged = async (config, program) => {
  try {
    ...
    const response = await sql
      `SELECT 'FG ON HAND TAGGED' AS column, ${sql(config.l1_field)} AS l1_label, COALESCE(${sql(config.l2_field)},'NA') AS l2_label, ${sql(config.l3_field)} AS l3_label, COALESCE(SUM(tagged_inventory.weight),0) AS lbs, COALESCE(SUM(tagged_inventory.cost * tagged_inventory.weight),0) AS cogs 
