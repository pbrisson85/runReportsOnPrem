...
const l0_getFgInven = async config => {
  try {
    ...
    const response = await sql
      `SELECT 'FG INVEN' AS column, REPLACE('${sql(config.itemType)} SALES','"','') AS l1_label, 'TOTAL' AS l2_label, 'TOTAL' AS l3_label, 'TOTAL' AS l4_label, COALESCE(SUM(perpetual_inventory.on_hand_lbs),0) AS lbs, COALESCE(SUM(perpetual_inventory.cost_extended),0) AS cogs 
...
const l0_getFgInTransit = async config => {
  try {
    ...
    const response = await sql
      `SELECT 'FG IN TRANSIT' AS column, REPLACE('${sql(config.itemType)} SALES','"','') AS l1_label, 'TOTAL' AS l2_label, 'TOTAL' AS l3_label, 'TOTAL' AS l4_label, COALESCE(SUM(perpetual_inventory.on_hand_lbs),0) AS lbs, COALESCE(SUM(perpetual_inventory.cost_extended),0) AS cogs 
...
const l0_getFgAtLoc = async config => {
  try {
    ...
    const response = await sql
      `SELECT 'FG ON HAND' AS column, REPLACE('${sql(config.itemType)} SALES','"','') AS l1_label, 'TOTAL' AS l2_label, 'TOTAL' AS l3_label, 'TOTAL' AS l4_label, COALESCE(SUM(perpetual_inventory.on_hand_lbs),0) AS lbs, COALESCE(SUM(perpetual_inventory.cost_extended),0) AS cogs 
...
const l0_getFgAtLoc_untagged = async config => {
  try {
    ...
    const response = await sql
 `SELECT 'FG ON HAND UNTAGGED' AS column, REPLACE('${sql(config.itemType)} SALES','"','') AS l1_label, 'TOTAL' AS l2_label, 'TOTAL' AS l3_label, 'TOTAL' AS l4_label, COALESCE(SUM(inven_t.on_hand_lbs),0) - COALESCE(SUM(tagged_t.weight),0) AS lbs , COALESCE(SUM(inven_t.cost_extended),0) - COALESCE(SUM(tagged_t.ext_cost),0) AS cogs 
...
const l0_getFgAtLoc_tagged = async config => {
  try {
    ...
    const response = await sql
    `SELECT 'FG ON HAND TAGGED' AS column, REPLACE('${sql(config.itemType)} SALES','"','') AS l1_label, 'TOTAL' AS l2_label, 'TOTAL' AS l3_label, 'TOTAL' AS l4_label, COALESCE(SUM(tagged_inventory.weight),0) AS lbs, COALESCE(SUM(tagged_inventory.cost * tagged_inventory.weight),0) AS cogs 
