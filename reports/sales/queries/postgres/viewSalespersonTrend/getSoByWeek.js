...
const lvl_1_subtotal_getSo_byWk = async config => {
  try {
    ...
    const response = await sql
         `SELECT so.week_serial || '_so' AS column, so.outside_salesperson_code AS l1_label, so.outside_salesperson_name AS l2_label, COALESCE(SUM(so.ext_weight),0) AS lbs, COALESCE(SUM(so.ext_sales),0) AS sales, COALESCE(SUM(so.ext_cost),0) AS cogs, COALESCE(SUM(so.ext_othp),0) AS othp 
        ...
        GROUP BY so.week_serial, so.outside_salesperson_code, so.outside_salesperson_name 
