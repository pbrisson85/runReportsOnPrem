...
const l0_getFgInven = async (config, trendQuery) => {
  try {
    ...
    const response = await sql
      `SELECT 'FG INVEN' AS column, '${sql(config.itemType)} SALES' AS l1_label, 'TOTAL' AS l2_label,  COALESCE(SUM(pi.on_hand_lbs),0) AS lbs, COALESCE(SUM(pi.cost_extended),0) AS cogs 
...
const l0_getFgInTransit = async (config, trendQuery) => {
  try {
    ...
    const response = await sql
      `SELECT 'FG IN TRANSIT' AS column, '${sql(config.itemType)} SALES' AS l1_label, 
...
const l0_getFgAtLoc = async (config, trendQuery) => {
  try {
    ...
    const response = await sql
      `SELECT 'FG ON HAND' AS column, '${sql(config.itemType)} SALES' AS l1_label, 
'TOTAL' AS l2_label,  COALESCE(SUM(pi.on_hand_lbs),0) AS lbs, COALESCE(SUM(pi.cost_extended),0) AS cogs 
...
const l0_getFgAtLoc_untagged = async (config, trendQuery) => {
  try {
    ...
    const response = await sql
 `SELECT 'FG ON HAND UNTAGGED' AS column, '${sql(config.itemType)} SALES' AS l1_label, 
...
const l0_getFgAtLoc_tagged = async (config, trendQuery) => {
  try {
    ...
    const response = await sql
      `SELECT 'FG ON HAND TAGGED' AS column, '${sql(config.itemType)} SALES' AS l1_label, 
