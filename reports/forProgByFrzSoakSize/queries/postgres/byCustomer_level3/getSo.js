...
const lvl_1_subtotal_getSo = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
         `SELECT \'FG OPEN ORDER\' AS column, sl.customer_code AS l1_label, sl.customer_name AS l2_label, COALESCE(SUM(so.ext_weight),0) AS lbs, COALESCE(SUM(so.ext_sales),0) AS sales, COALESCE(SUM(so.ext_cost),0) AS cogs, COALESCE(SUM(so.ext_othp),0) AS othp 
         
         FROM "salesReporting".sales_orders AS so
          LEFT OUTER JOIN "invenReporting".master_supplement AS ms 
            ON ms.item_num = so.item_num 
            
        WHERE ms.item_type = $1 AND so.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.program = $2 AND ms.byproduct_type IS NULL AND ms.fg_fresh_frozen = $3 AND ms.fg_treatment = $4 AND ms.size_name = $5 
        
        GROUP BY sl.customer_code, sl.customer_name`, ['FG', program, filters[0], filters[1], filters[2]]
        ) //prettier-ignore
