...
const lvl_1_subtotal_getFgInven_detail = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
          ...
          WHERE pi.on_hand_lbs <> 0 AND ms.byproduct_type IS NULL AND ms.item_type = $1 AND pi.version = (SELECT MAX(perpetual_inventory.version) - 1 FROM "invenReporting".perpetual_inventory) AND ms.program = $2 AND ms.fg_fresh_frozen = $3`,
...
const lvl_1_subtotal_getFgInTransit_detail = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
          ...
          WHERE pi.on_hand_lbs <> 0 AND ms.byproduct_type IS NULL AND ms.item_type = $1 AND pi.version = (SELECT MAX(perpetual_inventory.version) - 1 FROM "invenReporting".perpetual_inventory) AND pi.location_type = $2 AND ms.program = $3 AND ms.fg_fresh_frozen = $4`,
    ...
    const response = await pgClient.query(
          ...
          FROM (
              SELECT pi.receipt_date, pi.location_date, pi.lot_text, pi.msc_cert_bool AS msc, pi.location_code, pi.item_number AS item, pi.description, pi.lot, ms.species, ms.brand, ms.size_name AS size, ms.fg_treatment AS soak, pi.on_hand_lbs AS lbs, pi.cost_lb,  pi.cost_extended AS cost_ext, pi.location_name AS location, pi.location_country as country, ms.fg_fresh_frozen AS fresh_frozen 
                  ...
                  FROM "invenReporting".perpetual_inventory) AND pi.location_type <> $2 AND ms.program = $3 AND ms.fg_fresh_frozen = $4) 
                  AS all_inven
...
const lvl_1_subtotal_getFgAtLoc_tagged_detail = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
          ...
          WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND ti.version = (SELECT MAX(tagged_inventory.version) - 1 FROM "salesReporting".tagged_inventory) AND ms.program = $2 AND ms.fg_fresh_frozen = $3`,
          ['FG', program, filters[0]]
        ) //prettier-ignore
...
const lvl_2_subtotal_getFgInven_detail = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
        ...
        WHERE pi.on_hand_lbs <> 0 AND ms.byproduct_type IS NULL AND ms.item_type = $1 AND pi.version = (SELECT MAX(perpetual_inventory.version) - 1 FROM "invenReporting".perpetual_inventory) AND ms.program = $2 AND ms.fg_fresh_frozen = $3 AND ms.fg_treatment = $4`,
...
const lvl_2_subtotal_getFgInTransit_detail = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
        ...
        WHERE pi.on_hand_lbs <> 0 AND ms.byproduct_type IS NULL AND ms.item_type = $1 AND pi.version = (SELECT MAX(perpetual_inventory.version) - 1 FROM "invenReporting".perpetual_inventory) AND pi.location_type = $2 AND ms.program = $3 AND ms.fg_fresh_frozen = $4 AND ms.fg_treatment = $5`,
  ...
  try {
    ...
    const response = await pgClient.query(
        ...
        LEFT OUTER JOIN (
            ...
            WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND ti.version = (SELECT MAX(tagged_inventory.version) - 1 FROM "salesReporting".tagged_inventory) AND ms.program = $3 AND ms.fg_fresh_frozen = $4 AND ms.fg_treatment = $5
            GROUP BY ti.location, ti.item_num, ti.lot) 
            AS tagged_inven 
...
const lvl_2_subtotal_getFgAtLoc_tagged_detail = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
        ...
        WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND ti.version = (SELECT MAX(tagged_inventory.version) - 1 FROM "salesReporting".tagged_inventory) AND ms.program = $2 AND ms.fg_fresh_frozen = $3 AND ms.fg_treatment = $4`,
        ['FG', program, filters[0], filters[1]]
      ) //prettier-ignore
...
const lvl_3_subtotal_getFgInven_detail = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
          ...
          WHERE pi.on_hand_lbs <> 0 AND ms.byproduct_type IS NULL AND ms.item_type = $1 AND pi.version = (SELECT MAX(perpetual_inventory.version) - 1 FROM "invenReporting".perpetual_inventory) AND ms.program = $2 AND ms.fg_fresh_frozen = $3 AND ms.fg_treatment = $4 AND ms.size_name = $5`,
...
const lvl_3_subtotal_getFgInTransit_detail = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
          ...
          WHERE pi.on_hand_lbs <> 0 AND ms.byproduct_type IS NULL AND ms.item_type = $1 AND pi.version = (SELECT MAX(perpetual_inventory.version) - 1 FROM "invenReporting".perpetual_inventory) AND pi.location_type = $2 AND ms.program = $3 AND ms.fg_fresh_frozen = $4 AND ms.fg_treatment = $5 AND ms.size_name = $6`,
  ...
  try {
    ...
    const response = await pgClient.query(
          ...
          LEFT OUTER JOIN (
              ...
              WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND ti.version = (SELECT MAX(tagged_inventory.version) - 1 FROM "salesReporting".tagged_inventory) AND ms.program = $3 AND ms.fg_fresh_frozen = $4 AND ms.fg_treatment = $5 AND ms.size_name = $6
              GROUP BY ti.location, ti.item_num, ti.lot)  
              AS tagged_inven 
...
const lvl_3_subtotal_getFgAtLoc_tagged_detail = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
          ...
          WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND ti.version = (SELECT MAX(tagged_inventory.version) - 1 FROM "salesReporting".tagged_inventory) AND ms.program = $2 AND ms.fg_fresh_frozen = $3 AND ms.fg_treatment = $4 AND ms.size_name = $5`,
          ['FG', program, filters[0], filters[1], filters[2]]
        ) //prettier-ignore
