...
const lvl_1_subtotal_getSalesByFyYtd = async (start, end, program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      `SELECT sl.fiscal_year AS column, ms.item_num AS l1_label, ms.description AS l2_label, ms.brand AS l3_label, ms.size_name AS l4_label, COALESCE(SUM(sl.calc_gm_rept_weight),0) AS lbs, COALESCE(SUM(sl.gross_sales_ext),0) AS sales, COALESCE(SUM(sl.cogs_ext_gl),0) AS cogs, COALESCE(SUM(sl.othp_ext),0) AS othp 
