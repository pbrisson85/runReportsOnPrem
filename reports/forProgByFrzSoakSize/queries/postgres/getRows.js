...
const getRowsThirdLevelDetail = async (start, end, program) => {
  try {
    ...
    const response = await pgClient.query(
        `SELECT ms.fg_fresh_frozen AS l1_label, ms.fg_treatment AS l2_label, ms.size_name AS l3_label 
          ...
          GROUP BY ms.fg_fresh_frozen, ms.fg_treatment, ms.size_name 
        
        UNION SELECT ms.fg_fresh_frozen AS l1_label, ms.fg_treatment AS l2_label, ms.size_name AS l3_label 
        ...
        UNION SELECT ms.fg_fresh_frozen AS l1_label, ms.fg_treatment AS l2_label, ms.size_name AS l3_label 
          ...
          WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND sales_orders.version = (SELECT MAX(sales_orders.version) - 1 
              
          FROM "salesReporting".sales_orders) AND ms.program = $2 
...
const getRowsSecondLevelDetail = async (start, end, program) => {
  try {
    ...
    const response = await pgClient.query(
        `SELECT ms.fg_fresh_frozen AS l1_label, ms.fg_treatment AS l2_label, \'SUBTOTAL\' AS l3_label 
          ...
          GROUP BY ms.fg_fresh_frozen, ms.fg_treatment 
        
        UNION SELECT ms.fg_fresh_frozen AS l1_label, ms.fg_treatment AS l2_label, \'SUBTOTAL\' AS l3_label 
