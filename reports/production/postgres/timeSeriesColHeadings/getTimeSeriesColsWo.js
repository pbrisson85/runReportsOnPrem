...
const getTrendColsWo = async (config, woActivityGroups) => {
  ...
  try {
    ...
    for (woActivity of woActivityGroups) {
      ...
      SELECT 
          ${sql(config.trends.queryGrouping)} || ${sql`'_REPLACE(${sql(woActivity)}','"', '''')`} AS "dataName", 
          ...
          'wo_' || ${sql`'_REPLACE(${sql(woActivity)}','"', '''')`} AS "colType" -- should match the colType as the col that drives what is seen on front end via double click
