  ...
  try {
    ...
    const response = await pgClient.query(
        ...
        UNION SELECT so.customer_code AS l1_label, so.customer_name AS l2_label 
          ...
          WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND ms.program = $2 AND so.version = (SELECT MAX(sales_orders.version) - 1 FROM "salesReporting".sales_orders) AND ms.fg_fresh_frozen = $3 AND ms.brand = $4 
    ...
    const response = await pgClient.query(
        ...
        UNION SELECT so.customer_code AS l1_label, so.customer_name AS l2_label 
          FROM "salesReporting".sales_orders AS so
            LEFT OUTER JOIN "invenReporting".master_supplement AS ms 
              ON ms.item_num = so.item_num 
          WHERE ms.byproduct_type IS NULL AND ms.item_type = $3 AND ms.program = $4 AND so.version = (SELECT MAX(sales_orders.version) - 1 FROM "salesReporting".sales_orders) AND ms.fg_fresh_frozen = $5 AND ms.brand = $6 
