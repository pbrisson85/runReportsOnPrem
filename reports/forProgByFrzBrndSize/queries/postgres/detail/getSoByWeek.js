...
const lvl_2_subtotal_getSoByWk_detail = async (program, filters, weekSerial) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.item_type = $1 AND so.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.program = $2 AND ms.byproduct_type IS NULL AND ms.fg_fresh_frozen = $3 AND ms.brand = $4 AND so.week_serial = $5`, ['FG', program, filters[0], filters[1], weekSerial]
    ) //prettier-ignore
...
const lvl_2_subtotal_getSoTagged_detail = async (program, filters, weekSerial) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.item_type = $1 AND so.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.program = $2 AND ms.byproduct_type IS NULL AND so.tagged_weight > 0 AND ms.fg_fresh_frozen = $3 AND ms.brand = $4 AND so.week_serial = $5`, ['FG', program, filters[0], filters[1], weekSerial]
    ) //prettier-ignore
...
const lvl_2_subtotal_getSoByWkUntagged_detail = async (program, filters, weekSerial) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      FROM "salesReporting".sales_orders) AND ms.program = $2 AND ms.byproduct_type IS NULL AND so.untagged_weight > 0 AND ms.fg_fresh_frozen = $3 AND ms.brand = $4 AND so.week_serial = $5`, ['FG', program, filters[0], filters[1], weekSerial]
    ) //prettier-ignore
...
const lvl_3_subtotal_getSoByWk_detail = async (program, filters, weekSerial) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.item_type = $1 AND so.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.program = $2 AND ms.byproduct_type IS NULL AND ms.fg_fresh_frozen = $3 AND ms.brand = $4 AND ms.size_name = $5 AND so.week_serial = $6`, ['FG', program, filters[0], filters[1], filters[2], weekSerial]
     ) //prettier-ignore
...
const lvl_3_subtotal_getSoTagged_detail = async (program, filters, weekSerial) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.item_type = $1 AND so.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.program = $2 AND ms.byproduct_type IS NULL AND so.tagged_weight > 0 AND ms.fg_fresh_frozen = $3 AND ms.brand = $4 AND ms.size_name = $5 AND so.week_serial = $6`, ['FG', program,  filters[0], filters[1], filters[2], weekSerial]
    ) //prettier-ignore
...
const lvl_3_subtotal_getSoByWkUntagged_detail = async (program, filters, weekSerial) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.item_type = $1 AND so.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.program = $2 AND ms.byproduct_type IS NULL AND so.untagged_weight > 0 AND ms.fg_fresh_frozen = $3 AND ms.brand = $4 AND ms.size_name = $5 AND so.week_serial = $6`, ['FG', program,  filters[0], filters[1], filters[2], weekSerial]
    ) //prettier-ignore
