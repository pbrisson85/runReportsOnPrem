...
const lvl_2_subtotal_getFgPo_detail = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.item_type = $1 AND p.on_order_lbs <> 0 AND p.version = (SELECT MAX(p2.version) - 1 FROM "invenReporting".perpetual_inventory AS p2) AND ms.program = $2 AND ms.fg_fresh_frozen =  $3 AND ms.brand = $4`, ['FG', program, filters[0], filters[1]]
      ) //prettier-ignore
...
const lvl_3_subtotal_getFgPo_detail = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.item_type = $1 AND p.on_order_lbs <> 0 AND p.version = (SELECT MAX(p2.version) - 1 FROM "invenReporting".perpetual_inventory AS p2) AND ms.program = $2 AND ms.fg_fresh_frozen =  $3 AND ms.brand = $4 AND ms.size_name = $5`, ['FG', program, filters[0], filters[1], filters[2]]
      ) //prettier-ignore
