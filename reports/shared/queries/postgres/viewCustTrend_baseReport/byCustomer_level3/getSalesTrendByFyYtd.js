...
const lvl_1_subtotal_getSalesByFyYtd = async (config, start, end, program, filters) => {
  try {
    ...
    const response = await sql
      ...
      WHERE ms.byproduct_type IS NULL AND ms.item_type = ${'FG'} ${program ? sql`AND ms.program = ${program}`: sql``} AND ${sql(config.l1_field)} = ${filters[0]} AND ${sql(config.l2_field)} = ${filters[1]} AND ${sql(config.l3_field)} = ${filters[2]} AND sl.week >= ${start} AND sl.week <= ${end} 
