  ...
  try {
    ...
    const response = await sql
        ...
        UNION SELECT so.customer_code AS l1_label, so.customer_name AS l2_label 
          ...
          WHERE ms.byproduct_type IS NULL AND ms.item_type = ${'FG'} ${program ? sql`AND ms.program = ${program}`: sql``} AND so.version = (SELECT MAX(sales_orders.version) - 1 FROM "salesReporting".sales_orders) AND ${sql(config.l1_field)} = ${filters[0]} 
