...
const lvl_1_subtotal_getSo = async item => {
  try {
    ...
    const response = await pgClient.query(
        ...
        WHERE ms.item_num = $1 AND so.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.byproduct_type IS NULL 
        
        GROUP BY so.customer_code, so.customer_name`, [item]
        ) //prettier-ignore
...
const lvl_0_total_getSo = async item => {
  try {
    ...
    const response = await pgClient.query(
          ...
          WHERE ms.item_num = $1 AND so.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.byproduct_type IS NULL`, [item]
          ) //prettier-ignore
