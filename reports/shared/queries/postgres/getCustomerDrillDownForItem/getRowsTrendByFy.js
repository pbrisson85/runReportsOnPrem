  ...
  try {
    ...
    const response = await pgClient.query(
        `SELECT sl.customer_code AS l1_label, sl.customer_name AS l2_label 
          ...
          WHERE ms.byproduct_type IS NULL AND ms.item_num = $1 
        ...
        UNION SELECT so.customer_code AS l1_label, so.customer_name AS l2_label 
          ...
          WHERE ms.byproduct_type IS NULL AND ms.item_num = $1 AND so.version = (SELECT MAX(sales_orders.version) - 1 FROM "salesReporting".sales_orders) 
