...
const lvl_3_subtotal_getSoByWk_detail = async (config, program, filters, weekSerial) => {
  try {
    ...
    const response = await sql
      ...
      WHERE ms.item_type = ${'FG'} AND so.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) ${program ? sql`AND ms.program = ${program}`: sql``} AND ms.byproduct_type IS NULL AND ${sql(config.l1_field)} =  ${filters[0]} AND ${sql(config.l2_field)} =  ${filters[1]} AND ${sql(config.l1_field)} =  ${filters[2]} AND so.week_serial = ${weekSerial} AND so.customer_code = ${filters[3]}` //prettier-ignore
...
const lvl_3_subtotal_getSoTagged_detail = async (config, program, filters, weekSerial) => {
  try {
    ...
    const response = await sql
      ...
      WHERE ms.item_type = ${'FG'} AND so.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) ${program ? sql`AND ms.program = ${program}`: sql``} AND ms.byproduct_type IS NULL AND so.tagged_weight > 0 AND ${sql(config.l1_field)} =  ${filters[0]} AND ${sql(config.l2_field)} =  ${filters[1]} AND ${sql(config.l1_field)} =  ${filters[2]} AND so.week_serial = ${weekSerial} AND so.customer_code = ${filters[3]}` //prettier-ignore
...
const lvl_3_subtotal_getSoByWkUntagged_detail = async (config, program, filters, weekSerial) => {
  try {
    ...
    const response = await sql
      ...
      WHERE ms.item_type = ${'FG'} AND so.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) ${program ? sql`AND ms.program = ${program}`: sql``} AND ms.byproduct_type IS NULL AND so.untagged_weight > 0 AND ${sql(config.l1_field)} =  ${filters[0]} AND ${sql(config.l2_field)} =  ${filters[1]} AND ${sql(config.l1_field)} =  ${filters[2]} AND so.week_serial = ${weekSerial} AND so.customer_code = ${filters[3]}` //prettier-ignore
