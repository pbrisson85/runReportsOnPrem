...
const lvl_0_total_getSalesPeriodToDate = async (config, start, end, program, filters) => {
  try {
    ...
    const response = await sql
      ...
      WHERE sales_line_items.formatted_invoice_date >= ${start} AND sales_line_items.formatted_invoice_date <= ${end} AND ms.byproduct_type IS NULL AND ms.item_type = ${'FG'} AND ms.program = ${program} AND ${sql(config.l1_field)} = ${filters[0]}` //prettier-ignore
