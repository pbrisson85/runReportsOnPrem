...
const lvl_1_subtotal_getSalesByFyYtd = async (config, start, end, program, filters) => {
  try {
    ...
    const response = await sql
      ...
      WHERE ms.byproduct_type IS NULL AND ms.item_type = ${'FG'} AND ms.program = ${program} AND ${sql(config.l1_field)} = ${filters[0]} AND sl.customer_code = ${filters[3]} AND sl.week >= ${start} AND sl.week <= ${end}
