...
const lvl_1_subtotal_getSo = async (config, program, filters) => {
  try {
    ...
    const response = await sql
         `SELECT 'FG OPEN ORDER' AS column, ms.item_num AS l1_label, ms.description AS l2_label, ${sql(config.l3_field)} AS l3_label, COALESCE(SUM(sales_orders.ext_weight),0) AS lbs, COALESCE(SUM(sales_orders.ext_sales),0) AS sales, COALESCE(SUM(sales_orders.ext_cost),0) AS cogs, COALESCE(SUM(sales_orders.ext_othp),0) AS othp 
         ...
         WHERE ms.item_type = ${'FG'} AND sales_orders.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.program = ${program} AND ms.byproduct_type IS NULL AND ${sql(config.l1_field)} = ${filters[0]} AND ${sql(config.l2_field)} = ${filters[1]} AND sales_orders.customer_code = ${filters[3]} 
...
const lvl_1_subtotal_getSoTagged = async (config, program, filters) => {
  try {
    ...
    const response = await sql
           ...
           WHERE ms.item_type = ${'FG'} AND sales_orders.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.program = ${program} AND ms.byproduct_type IS NULL AND sales_orders.tagged_weight > 0 AND ${sql(config.l1_field)} = ${filters[0]} AND ${sql(config.l2_field)} = ${filters[1]} AND sales_orders.customer_code = ${filters[3]} 
...
const lvl_1_subtotal_getSoUntagged = async (config, program, filters) => {
  try {
    ...
    const response = await sql
      ...
      WHERE ms.item_type = ${'FG'} AND sales_orders.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.program = ${program} AND ms.byproduct_type IS NULL AND sales_orders.untagged_weight > 0 AND ${sql(config.l1_field)} = ${filters[0]} AND ${sql(config.l2_field)} = ${filters[1]} AND sales_orders.customer_code = ${filters[3]} 
...
const lvl_0_total_getSo = async (config, program, filters) => {
  try {
    ...
    const response = await sql
           ...
           WHERE ms.item_type = ${'FG'} AND sales_orders.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.program = ${program} AND ms.byproduct_type IS NULL AND ${sql(config.l1_field)} = ${filters[0]} AND ${sql(config.l2_field)} = ${filters[1]} AND sales_orders.customer_code = ${filters[3]}` //prettier-ignore
...
const lvl_0_total_getSoTagged = async (config, program, filters) => {
  try {
    ...
    const response = await sql
      ...
      WHERE ms.item_type = ${'FG'} AND sales_orders.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.program = ${program} AND ms.byproduct_type IS NULL AND sales_orders.tagged_weight > 0 AND ${sql(config.l1_field)} = ${filters[0]} AND ${sql(config.l2_field)} = ${filters[1]} AND sales_orders.customer_code = ${filters[3]}` //prettier-ignore
...
const lvl_0_total_getSoUntagged = async (config, program, filters) => {
  try {
    ...
    const response = await sql
      ...
      WHERE ms.item_type = ${'FG'} AND sales_orders.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.program = ${program} AND ms.byproduct_type IS NULL AND sales_orders.untagged_weight > 0 AND ${sql(config.l1_field)} = ${filters[0]} AND ${sql(config.l2_field)} = ${filters[1]} AND sales_orders.customer_code = ${filters[3]}` //prettier-ignore
