...
const lvl_1_subtotal_getSalesByWk = async (config, start, end, program, filters) => {
  try {
    console.log(`level 1: (getSalesTrend Lvl1) query postgres to get FG sales data by week ...`)
    ...
    const response = await sql
      ...
      WHERE sales_line_items.formatted_invoice_date >= ${start} AND sales_line_items.formatted_invoice_date <= ${end} AND ms.byproduct_type IS NULL AND ms.item_type = ${'FG'} ${program ? sql`AND ms.program = ${program}`: sql``} AND ${sql(config.l1_field)} = ${filters[0]} AND sales_line_items.customer_code = ${filters[3]}
...
const lvl_1_subtotal_getSalesPeriodToDate = async (config, start, end, program, filters) => {
  try {
    console.log(`level 1: (getSalesTrend Lvl1) query postgres to get FG sales data period total ...`)
...
const lvl_0_total_getSalesByWk = async (config, start, end, program, filters) => {
  try {
    console.log(`level 0: (getSalesTrend Lvl1) query postgres to get FG sales data by week ...`)
...
const lvl_0_total_getSalesPeriodToDate = async (config, start, end, program, filters) => {
  try {
    console.log(`level 0: (getSalesTrend Lvl1) query postgres to get FG sales data period total ...`)
    ...
    const response = await sql
      ...
      WHERE sales_line_items.formatted_invoice_date >= ${start} AND sales_line_items.formatted_invoice_date <= ${end} AND ms.byproduct_type IS NULL AND ms.item_type = ${'FG'} ${program ? sql`AND ms.program = ${program}`: sql``} AND ${sql(config.l1_field)} = ${filters[0]} AND sales_line_items.customer_code = ${filters[3]}` //prettier-ignore
