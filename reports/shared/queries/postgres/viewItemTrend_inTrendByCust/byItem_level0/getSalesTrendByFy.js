...
const lvl_1_subtotal_getSalesByFy = async (config, start, end, program, filters) => {
  try {
    ...
    const response = await sql
      ...
      WHERE ms.byproduct_type IS NULL AND ms.item_type = ${'FG'} ${program ? sql`AND ms.program = ${program}`: sql``} AND sales_line_items.customer_code = ${filters[3]}
      
      GROUP BY sales_line_items.fiscal_year, ms.item_num, ms.description, ${sql(config.l1_field)}, ${sql(config.l2_field)}, ${sql(config.l3_field)} 
