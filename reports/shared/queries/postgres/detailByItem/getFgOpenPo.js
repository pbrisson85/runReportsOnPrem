...
const byItem_getFgPo_detail = async itemCode => {
  try {
    ...
    const response = await pgClient.query(
        ...
        WHERE ms.item_num = $1 AND p.on_order_lbs <> 0 AND p.version = (SELECT MAX(p2.version) - 1 FROM "invenReporting".perpetual_inventory AS p2) `, [itemCode]
        ) //prettier-ignore
