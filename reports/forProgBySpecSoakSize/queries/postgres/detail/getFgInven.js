...
const lvl_2_subtotal_getFgInven_detail = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
        ...
        WHERE pi.on_hand_lbs <> 0 AND ms.byproduct_type IS NULL AND ms.item_type = $1 AND pi.version = (SELECT MAX(perpetual_inventory.version) - 1 FROM "invenReporting".perpetual_inventory) AND ms.program = $2 AND ms.species = $3 AND ms.fg_treatment = $4`,
...
const lvl_2_subtotal_getFgInTransit_detail = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
        ...
        WHERE pi.on_hand_lbs <> 0 AND ms.byproduct_type IS NULL AND ms.item_type = $1 AND pi.version = (SELECT MAX(perpetual_inventory.version) - 1 FROM "invenReporting".perpetual_inventory) AND pi.location_type = $2 AND ms.program = $3 AND ms.species = $4 AND ms.fg_treatment = $5`,
...
const lvl_2_subtotal_getFgAtLoc_tagged_detail = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
        ...
        WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND ti.version = (SELECT MAX(tagged_inventory.version) - 1 FROM "salesReporting".tagged_inventory) AND ms.program = $2 AND ms.species = $3 AND ms.fg_treatment = $4`,
        ['FG', program, filters[0], filters[1]]
      ) //prettier-ignore
...
const lvl_3_subtotal_getFgInven_detail = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
          ...
          WHERE pi.on_hand_lbs <> 0 AND ms.byproduct_type IS NULL AND ms.item_type = $1 AND pi.version = (SELECT MAX(perpetual_inventory.version) - 1 FROM "invenReporting".perpetual_inventory) AND ms.program = $2 AND ms.species = $3 AND ms.fg_treatment = $4 AND ms.size_name = $5`,
...
const lvl_3_subtotal_getFgInTransit_detail = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
          ...
          WHERE pi.on_hand_lbs <> 0 AND ms.byproduct_type IS NULL AND ms.item_type = $1 AND pi.version = (SELECT MAX(perpetual_inventory.version) - 1 FROM "invenReporting".perpetual_inventory) AND pi.location_type = $2 AND ms.program = $3 AND ms.species = $4 AND ms.fg_treatment = $5 AND ms.size_name = $6`,
...
const lvl_3_subtotal_getFgAtLoc_tagged_detail = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
          ...
          WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND ti.version = (SELECT MAX(tagged_inventory.version) - 1 FROM "salesReporting".tagged_inventory) AND ms.program = $2 AND ms.species = $3 AND ms.fg_treatment = $4 AND ms.size_name = $5`,
          ['FG', program, filters[0], filters[1], filters[2]]
        ) //prettier-ignore
