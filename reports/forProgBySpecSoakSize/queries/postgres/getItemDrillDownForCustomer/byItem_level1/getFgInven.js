...
const lvl_1_subtotal_getFgInven = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      `SELECT \'FG INVEN\' AS column, ms.item_num AS l1_label, ms.description AS l2_label, ms.fg_treatment AS l3_label , ms.size_name AS l4_label, COALESCE(SUM(perpetual_inventory.on_hand_lbs),0) AS lbs, COALESCE(SUM(perpetual_inventory.cost_extended),0) AS cogs 
...
const lvl_1_subtotal_getFgInTransit = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      `SELECT \'FG IN TRANSIT\' AS column, ms.item_num AS l1_label, ms.description AS l2_label, ms.fg_treatment AS l3_label , ms.size_name AS l4_label, COALESCE(SUM(perpetual_inventory.on_hand_lbs),0) AS lbs, COALESCE(SUM(perpetual_inventory.cost_extended),0) AS cogs 
...
const lvl_1_subtotal_getFgAtLoc = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      `SELECT \'FG ON HAND\' AS column, ms.item_num AS l1_label, ms.description AS l2_label, ms.fg_treatment AS l3_label , ms.size_name AS l4_label, COALESCE(SUM(perpetual_inventory.on_hand_lbs),0) AS lbs, COALESCE(SUM(perpetual_inventory.cost_extended),0) AS cogs 
...
const lvl_1_subtotal_getFgAtLoc_untagged = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
 `SELECT \'FG ON HAND UNTAGGED\' AS column, ms.item_num AS l1_label, ms.description AS l2_label, ms.fg_treatment AS l3_label , ms.size_name AS l4_label, COALESCE(SUM(inven_t.on_hand_lbs),0) - COALESCE(SUM(tagged_t.weight),0) AS lbs , COALESCE(SUM(inven_t.cost_extended),0) - COALESCE(SUM(tagged_t.ext_cost),0) AS cogs 
...
const lvl_1_subtotal_getFgAtLoc_tagged = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      `SELECT \'FG ON HAND TAGGED\' AS column, ms.item_num AS l1_label, ms.description AS l2_label, ms.fg_treatment AS l3_label , ms.size_name AS l4_label, COALESCE(SUM(tagged_inventory.weight),0) AS lbs, COALESCE(SUM(tagged_inventory.cost * tagged_inventory.weight),0) AS cogs 
