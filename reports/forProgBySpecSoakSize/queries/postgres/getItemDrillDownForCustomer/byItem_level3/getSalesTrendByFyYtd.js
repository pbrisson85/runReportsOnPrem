...
const lvl_1_subtotal_getSalesByFyYtd = async (start, end, program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.byproduct_type IS NULL AND ms.item_type = $3 AND ms.program = $4 AND ms.species = $5 AND ms.fg_treatment = $6 AND ms.size_name = $7 AND sl.customer_code = $8 AND sl.week >= $1 AND sl.week <= $2 
