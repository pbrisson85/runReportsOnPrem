...
const lvl_1_subtotal_getFgInven = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND perpetual_inventory.version = (SELECT MAX(perpetual_inventory.version) - 1 FROM "invenReporting".perpetual_inventory) AND ms.program = $2 AND ms.species = $3 AND ms.brand = $4 
...
const lvl_1_subtotal_getFgInTransit = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND perpetual_inventory.version = (SELECT MAX(perpetual_inventory.version) - 1 FROM "invenReporting".perpetual_inventory) AND perpetual_inventory.location_type = $2 AND ms.program = $3 AND ms.species = $4 AND ms.brand = $5 
...
WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND ms.program = $3 AND ms.species = $4 AND ms.brand = $5 

GROUP BY ms.item_num, ms.description, ms.size_name`, ['FG', 'IN TRANSIT', program, filters[0], filters[1]]
  ) //prettier-ignore
...
const lvl_1_subtotal_getFgAtLoc_tagged = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND tagged_inventory.version = (SELECT MAX(tagged_inventory.version) - 1 FROM "salesReporting".tagged_inventory) AND ms.program = $2 AND ms.species = $3 AND ms.brand = $4 
...
const lvl_0_total_getFgInTransit = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND perpetual_inventory.version = (SELECT MAX(perpetual_inventory.version) - 1 FROM "invenReporting".perpetual_inventory) AND ms.program = $2 AND perpetual_inventory.location_type = $3 AND ms.species = $4 AND ms.brand = $5`,
