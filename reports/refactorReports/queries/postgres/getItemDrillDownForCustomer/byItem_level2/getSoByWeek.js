...
const lvl_1_subtotal_getSo_byWk = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
         ...
         ORDER BY sales_orders.week_serial`, ['FG', program, filters[0], filters[1], filters[3]]
        ) //prettier-ignore
...
const lvl_0_total_getSo_byWk = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
           ...
           WHERE ms.item_type = $1 AND sales_orders.version = (SELECT MAX(version) - 1 
           
           FROM "salesReporting".sales_orders) AND ms.program = $2 AND ms.byproduct_type IS NULL AND ms.species = $3 AND ms.brand = $4 AND sales_orders.customer_code = $5 
