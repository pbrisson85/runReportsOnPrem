  ...
  try {
    ...
    const response = await pgClient.query(
        `SELECT ms.item_num AS l1_label, ms.description AS l2_label, ms.size_name AS l3_label 
          ...
          WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND ms.program = $2 AND ms.species = $3 AND ms.brand = $4 AND sales_line_items.customer_code = $5
        ...
        UNION SELECT ms.item_num AS l1_label, ms.description AS l2_label, ms.size_name AS l3_label 
          ...
          WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND ms.program = $2 AND sales_orders.version = (SELECT MAX(sales_orders.version) - 1 FROM "salesReporting".sales_orders) AND ms.species = $3 AND ms.brand = $4 AND sales_orders.customer_code = $5 
