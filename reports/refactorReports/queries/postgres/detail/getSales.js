...
const lvl_2_subtotal_getSales_detail = async (start, end, program, filters, year) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE sl.week >= $1 AND sl.week <= $2 AND ms.byproduct_type IS NULL AND ms.item_type = $3 AND ms.program = $4 AND ms.species = $5 AND ms.brand = $6 AND sl.fiscal_year = $7`,
...
const lvl_3_subtotal_getSales_detail = async (start, end, program, filters, year) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE sl.week >= $1 AND sl.week <= $2 AND ms.byproduct_type IS NULL AND ms.item_type = $3 AND ms.program = $4 AND ms.species = $5 AND ms.brand = $6 AND ms.size_name = $7 AND sl.fiscal_year = $8`,
