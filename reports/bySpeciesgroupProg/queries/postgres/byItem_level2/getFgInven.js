...
const lvl_1_subtotal_getFgInven = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND perpetual_inventory.version = (SELECT MAX(perpetual_inventory.version) - 1 FROM "invenReporting".perpetual_inventory) AND ms.species_group = $2 AND ms.program = $3 
...
const lvl_1_subtotal_getFgInTransit = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND perpetual_inventory.version = (SELECT MAX(perpetual_inventory.version) - 1 FROM "invenReporting".perpetual_inventory) AND perpetual_inventory.location_type = $2 AND ms.species_group = $3 AND ms.program = $4 
...
const lvl_1_subtotal_getFgAtLoc_untagged = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND ms.species_group = $3 AND ms.program = $4 
      
      GROUP BY ms.item_num, ms.description, ms.size_name`, ['FG', 'IN TRANSIT', filters[0], filters[1]]
    ) //prettier-ignore
...
const lvl_1_subtotal_getFgAtLoc_tagged = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND tagged_inventory.version = (SELECT MAX(tagged_inventory.version) - 1 FROM "salesReporting".tagged_inventory) AND ms.species_group = $2 AND ms.program = $3 
    ...
    const response = await pgClient.query(
      ...
      FROM "invenReporting".perpetual_inventory 
        LEFT OUTER JOIN "invenReporting".master_supplement 
          AS ms ON ms.item_num = perpetual_inventory.item_number 
...
const lvl_0_total_getFgAtLoc_untagged = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      `SELECT \'FG ON HAND UNTAGGED\' AS column, \'FG SALES\' AS l1_label, COALESCE(SUM(inven_t.on_hand_lbs),0) - COALESCE(SUM(tagged_t.weight),0) AS lbs , COALESCE(SUM(inven_t.cost_extended),0) - COALESCE(SUM(tagged_t.ext_cost),0) AS cogs FROM (SELECT pi.cost_extended, pi.item_number, pi.lot, pi.on_hand_lbs, pi.location_code 
      ...
      WHERE ms.byproduct_type IS NULL AND ms.item_type = $1 AND ms.species_group = $3 AND ms.program = $4`, ['FG', 'IN TRANSIT', filters[0], filters[1]]
    ) //prettier-ignore
