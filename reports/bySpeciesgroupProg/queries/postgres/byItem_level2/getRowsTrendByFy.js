...
NOTE THAT PREVIOUSLY GET ROWS WOULD FILTER BY DATE BUT SONCE I AM SHOWING ALL YEARS IN THE MAIN SALES TREND THEN WE NEED ALL YEARS WORTH OF DATA. BELOW WAS THE ORIGINAL QUERY WHICH I MAY NEED TO GO BACK TO.
...
const response = await pgClient.query(
        `SELECT ms.item_num AS l1_label, ms.description AS l2_label, ms.size_name AS l3_label 
            ...
            WHERE sales_line_items.formatted_invoice_date >= $1 AND sales_line_items.formatted_invoice_date <= $2 AND ms.byproduct_type IS NULL AND ms.item_type = $3 AND ms.species_group = $4 AND ms.program = $5 
