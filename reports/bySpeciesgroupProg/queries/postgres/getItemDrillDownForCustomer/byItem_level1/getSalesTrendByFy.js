...
const lvl_1_subtotal_getSalesByFy = async filters => {
  try {
    ...
    const response = await pgClient.query(
      `SELECT sales_line_items.fiscal_year AS column, ms.item_num AS l1_label, ms.description AS l2_label, ms.fg_treatment AS l3_label, ms.size_name AS l4_label, COALESCE(SUM(sales_line_items.calc_gm_rept_weight),0) AS lbs, COALESCE(SUM(sales_line_items.gross_sales_ext),0) AS sales, COALESCE(SUM(sales_line_items.cogs_ext_gl),0) AS cogs, COALESCE(SUM(sales_line_items.othp_ext),0) AS othp 
      ...
      GROUP BY sales_line_items.fiscal_year, ms.item_num, ms.description, ms.fg_treatment, ms.size_name 
...
const lvl_1_subtotal_getSalesByFyYtd = async (start, end, filters) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE  ms.byproduct_type IS NULL AND ms.item_type = $3 AND ms.species_group = $4 AND sales_line_items.customer_code = $5 AND sales_line_items.formatted_invoice_date >= $1 AND sales_line_items.formatted_invoice_date <= $2
