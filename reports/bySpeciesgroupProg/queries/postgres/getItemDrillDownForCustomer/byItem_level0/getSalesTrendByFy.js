...
const lvl_1_subtotal_getSalesByFy = async filters => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE  ms.byproduct_type IS NULL AND ms.item_type = $1 AND sales_line_items.customer_code = $2
      
      GROUP BY sales_line_items.fiscal_year, ms.item_num, ms.description, ms.fg_fresh_frozen, ms.fg_treatment, ms.size_name 
...
const lvl_1_subtotal_getSalesByFyYtd = async (start, end, filters) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE  ms.byproduct_type IS NULL AND ms.item_type = $3 AND sales_line_items.customer_code = $4 AND sales_line_items.formatted_invoice_date >= $1 AND sales_line_items.formatted_invoice_date <= $2
