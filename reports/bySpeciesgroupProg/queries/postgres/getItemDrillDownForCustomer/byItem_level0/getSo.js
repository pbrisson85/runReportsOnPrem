...
const lvl_1_subtotal_getSo = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
        ...
        WHERE ms.item_type = $1 AND sales_orders.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.byproduct_type IS NULL AND sales_orders.customer_code = $2
        
        GROUP BY ms.item_num, ms.description,ms.fg_fresh_frozen, ms.fg_treatment, ms.size_name`, ['FG', filters[3]]
        ) //prettier-ignore
...
const lvl_1_subtotal_getSoTagged = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
          ...
          WHERE ms.item_type = $1 AND sales_orders.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.byproduct_type IS NULL AND sales_orders.tagged_weight > 0 AND sales_orders.customer_code = $2 
...
const lvl_1_subtotal_getSoUntagged = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.item_type = $1 AND sales_orders.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.byproduct_type IS NULL AND sales_orders.untagged_weight > 0 AND sales_orders.customer_code = $2
...
const lvl_0_total_getSo = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.item_type = $1 AND sales_orders.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.byproduct_type IS NULL AND sales_orders.customer_code = $2`, ['FG', filters[3]]
    ) //prettier-ignore
...
const lvl_0_total_getSoTagged = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.item_type = $1 AND sales_orders.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.byproduct_type IS NULL AND sales_orders.tagged_weight > 0 AND sales_orders.customer_code = $2`, ['FG', filters[3]]
          ) //prettier-ignore
...
const lvl_0_total_getSoUntagged = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.item_type = $1 AND sales_orders.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.byproduct_type IS NULL AND sales_orders.untagged_weight > 0 AND sales_orders.customer_code = $2`, ['FG', filters[3]]
          ) //prettier-ignore
