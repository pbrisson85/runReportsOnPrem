...
const lvl_1_subtotal_getSalesByFy = async filters => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE  ms.byproduct_type IS NULL AND ms.item_type = $1 AND ms.species_group = $2 AND ms.program = $3 AND sales_line_items.customer_code = $4 
      
      GROUP BY sales_line_items.fiscal_year, ms.item_num, ms.description, ms.size_name 
      
      ORDER BY sales_line_items.fiscal_year`,
      [ 'FG', filters[0], filters[1], filters[3]]
      ) //prettier-ignore
...
const lvl_1_subtotal_getSalesByFyYtd = async (start, end, filters) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE  ms.byproduct_type IS NULL AND ms.item_type = $3 AND ms.species_group = $4 AND ms.program = $5 AND sales_line_items.customer_code = $6 AND sales_line_items.formatted_invoice_date >= $1 AND sales_line_items.formatted_invoice_date <= $2 
