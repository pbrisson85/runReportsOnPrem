...
const lvl_1_subtotal_getSo_byWk = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
        ...
        ORDER BY sales_orders.week_serial`, ['FG', filters[0], filters[1], filters[3]]
        ) //prettier-ignore
...
const lvl_1_subtotal_getSoTagged_byWk = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
          ...
          WHERE ms.item_type = $1 AND sales_orders.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.byproduct_type IS NULL AND sales_orders.tagged_weight > 0 AND ms.species_group = $2 AND ms.program = $3 AND sales_orders.customer_code = $4 
...
const lvl_1_subtotal_getSoUntagged_byWk = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.item_type = $1 AND sales_orders.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.byproduct_type IS NULL AND sales_orders.untagged_weight > 0 AND ms.species_group = $2 AND ms.program = $3 AND sales_orders.customer_code = $4 
...
const lvl_0_total_getSo_byWk = async (program, filters) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      FROM "salesReporting".sales_orders 
        LEFT OUTER JOIN "invenReporting".master_supplement AS ms 
          ON ms.item_num = sales_orders.item_num WHERE ms.item_type = $1 AND sales_orders.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.byproduct_type IS NULL AND ms.species_group = $2 AND ms.program = $3 AND sales_orders.customer_code = $4 
