...
const getLevelTwoRows = async (start, end) => {
  try {
    ...
    const response = await pgClient.query(
        'SELECT ms.species_group AS l1_subtotal, ms.program AS l2_subtotal FROM "salesReporting".sales_line_items LEFT OUTER JOIN "invenReporting".master_supplement AS ms ON ms.item_num = sales_line_items.item_number WHERE sales_line_items.formatted_invoice_date >= $1 AND sales_line_items.formatted_invoice_date <= $2 AND ms.byproduct_type IS NULL AND ms.item_type = $3 GROUP BY ms.species_group, ms.program UNION SELECT ms.species_group AS l1_subtotal, ms.program AS l2_subtotal FROM "invenReporting".perpetual_inventory LEFT OUTER JOIN "invenReporting".master_supplement AS ms ON ms.item_num = perpetual_inventory.item_number WHERE ms.byproduct_type IS NULL AND ms.item_type = $3 AND perpetual_inventory.version = (SELECT MAX(perpetual_inventory.version) - 1 FROM "invenReporting".perpetual_inventory) GROUP BY ms.species_group, ms.program UNION SELECT ms.species_group AS l1_subtotal, ms.program AS l2_subtotal FROM "salesReporting".sales_orders LEFT OUTER JOIN "invenReporting".master_supplement AS ms ON ms.item_num = sales_orders.item_num WHERE ms.byproduct_type IS NULL AND ms.item_type = $3 AND sales_orders.version = (SELECT MAX(sales_orders.version) - 1 FROM "salesReporting".sales_orders) GROUP BY ms.species_group, ms.program',
        [start, end, 'FG']
        ) //prettier-ignore
