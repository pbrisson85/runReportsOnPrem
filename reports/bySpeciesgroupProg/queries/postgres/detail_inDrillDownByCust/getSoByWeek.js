...
const lvl_1_subtotal_getSoByWk_detail = async (program, filters, weekSerial) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.item_type = $1 AND so.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.byproduct_type IS NULL AND ms.species_group = $2 AND so.week_serial = $3 AND so.customer_code = $4`, ['FG', filters[0], weekSerial, filters[3]]
     ) //prettier-ignore
  ...
  try {
    ...
    const response = await pgClient.query(
     `SELECT so.no_cost_found, so.customer_code, so.customer_name, so.so_num, so.location, so.so_line, so.formatted_ship_date, so.week_serial, so.item_num, ms.description, ms.species, ms.brand, ms.size_name, ms.fg_treatment, ms.fg_fresh_frozen, so.tagged_weight AS lbs, COALESCE(so.tagged_weight * so.unit_price,0) AS ext_sales, COALESCE(so.tagged_weight * so.othp_lb,0) AS ext_othp, COALESCE(so.tagged_weight * ave_tagged_cost,0) AS ext_cost, COALESCE(so.tagged_weight * so.unit_price - so.tagged_weight * so.othp_lb - so.tagged_weight * ave_tagged_cost,0) AS gross_margin_ext, so.unit_price, so.rebate_lb, so.discount_lb, so.freight_lb, so.othp_lb, so.sales_net_lb, so.ave_tagged_cost AS cost_lb, COALESCE(so.sales_net_lb - so.ave_tagged_cost,0) AS gross_margin_lb, so.tagged_weight, so.untagged_weight, so.ave_tagged_cost, so.ave_untagged_cost, so.ext_comm, so.commission_lb, so.sales_net_ext, so.ext_rebate, so.ext_discount, so.ext_freight, so.cost_ext_tagged, so.cost_ext_untagged
      ...
      WHERE ms.item_type = $1 AND so.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.byproduct_type IS NULL AND so.tagged_weight > 0 AND ms.species_group = $2 AND so.week_serial = $3 AND so.customer_code = $4`, ['FG', filters[0], weekSerial, filters[3]]
   ) //prettier-ignore
...
const lvl_1_subtotal_getSoByWkUntagged_detail = async (program, filters, weekSerial) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.item_type = $1 AND so.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.byproduct_type IS NULL AND so.untagged_weight > 0 AND ms.species_group = $2 AND so.week_serial = $3 AND so.customer_code = $4`, ['FG', filters[0], weekSerial, filters[3]]
    ) //prettier-ignore
...
const lvl_2_subtotal_getSoByWk_detail = async (program, filters, weekSerial) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.item_type = $1 AND so.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.byproduct_type IS NULL AND ms.species_group = $2 AND ms.program = $3 AND so.week_serial = $4 AND so.customer_code = $5`, ['FG', filters[0], filters[1], weekSerial, filters[3]]
    ) //prettier-ignore
...
const lvl_2_subtotal_getSoTagged_detail = async (program, filters, weekSerial) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.item_type = $1 AND so.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.byproduct_type IS NULL AND so.tagged_weight > 0 AND ms.species_group = $2 AND ms.program = $3 AND so.week_serial = $4 AND so.customer_code = $5`, ['FG', filters[0], filters[1], weekSerial, filters[3]]
    ) //prettier-ignore
...
const lvl_2_subtotal_getSoByWkUntagged_detail = async (program, filters, weekSerial) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      FROM "salesReporting".sales_orders) AND ms.byproduct_type IS NULL AND so.untagged_weight > 0 AND ms.species_group = $2 AND ms.program = $3 AND so.week_serial = $4 AND so.customer_code = $5`, ['FG', filters[0], filters[1], weekSerial, filters[3]]
    ) //prettier-ignore
...
const lvl_0_total_getSoByWk_detail = async (program, filters, weekSerial) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.item_type = $1 AND so.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.byproduct_type IS NULL AND so.week_serial = $2 AND so.customer_code = $3`, ['FG', weekSerial, filters[3]]
    ) //prettier-ignore
...
const lvl_0_total_getSoTagged_detail = async (program, filters, weekSerial) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.item_type = $1 AND so.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.byproduct_type IS NULL AND so.tagged_weight > 0 AND so.week_serial = $2 AND so.customer_code = $3`, ['FG', weekSerial, filters[3]]
    ) //prettier-ignore
...
const lvl_0_total_getSoByWkUntagged_detail = async (program, filters, weekSerial) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE ms.item_type = $1 AND so.version = (SELECT MAX(version) - 1 FROM "salesReporting".sales_orders) AND ms.byproduct_type IS NULL AND so.untagged_weight > 0 AND so.week_serial = $2 AND so.customer_code = $3`, ['FG', weekSerial, filters[3]]
    ) //prettier-ignore
