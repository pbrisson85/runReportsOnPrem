...
const lvl_1_subtotal_getSales_detail = async (startWeek, endWeek, program, filters, year) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE sl.week >= $1 AND sl.week <= $2 AND ms.byproduct_type IS NULL AND ms.item_type = $3 AND ms.species_group = $4 AND sl.fiscal_year = $5`,
      [startWeek, endWeek, 'FG', filters[0], year]
      ) //prettier-ignore
...
const lvl_2_subtotal_getSales_detail = async (startWeek, endWeek, program, filters, year) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE sl.week >= $1 AND sl.week <= $2 AND ms.byproduct_type IS NULL AND ms.item_type = $3 AND ms.species_group = $4 AND ms.program = $5 AND sl.fiscal_year = $6`,
      [startWeek, endWeek, 'FG', filters[0], filters[1], year]
      ) //prettier-ignore
...
const lvl_0_total_getSales_detail = async (startWeek, endWeek, program, filters, year) => {
  try {
    ...
    const response = await pgClient.query(
      ...
      WHERE sl.week >= $1 AND sl.week <= $2 AND ms.byproduct_type IS NULL AND ms.item_type = $3 AND sl.fiscal_year = $4`,
      [startWeek, endWeek, 'FG', year]
    ) //prettier-ignore
