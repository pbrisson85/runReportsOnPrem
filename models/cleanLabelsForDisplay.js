...
const cleanLabelsForDisplay = (flattenedMappedData, config) => {
  ...
  cacheData.forEach((row, idx) => {
    ...
    flattenedMappedData[idx].l5_filter = row.l5_label
    ...
    // If l4 grouping does not change then don't show it
    if (row.l4_label === l4Value) {
    ...
    if (row.datalevel === 1) {
      ...
      if (collapsedLabels) {
        ...
        flattenedMappedData[idx].l5_label = maxLevel === 5 ? `${dataLabel}${subtotalLabel}` : ''
      } else if (shiftTotals || shiftTotalsCss) {
        flattenedMappedData[idx].l1_label = ''
        flattenedMappedData[idx].l2_label = `${dataLabel}${subtotalLabel}`
        flattenedMappedData[idx].l3_label = ''
        flattenedMappedData[idx].l4_label = ''
        flattenedMappedData[idx].l5_label = ''
      } else {
    ...
    if (row.datalevel === 4 && config.baseFormat.l5_field) {
      ...
      if (collapsedLabels) {
        ...
        flattenedMappedData[idx].l3_label = ''
        flattenedMappedData[idx].l4_label = ''
        flattenedMappedData[idx].l5_label = `${dataLabel}${subtotalLabel}`
      } else {
        flattenedMappedData[idx].l1_label = ''
        flattenedMappedData[idx].l2_label = ''
        flattenedMappedData[idx].l3_label = ''
      ...
      if (shiftTotalsCss) {
        flattenedMappedData[idx].l5_total = true
        flattenedMappedData[idx].shiftTotalsCss = true
      } else {
        flattenedMappedData[idx].l4_total = true
      }
    ...
    // if l4 grouping does change, set new value
    if (row.l4_label !== l4Value) {
      l4Value = row.l4_label
    }
  })
