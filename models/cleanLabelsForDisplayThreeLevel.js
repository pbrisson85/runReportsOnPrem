...
const cleanLabelsForDisplay = flattenedMappedData => {
  ...
  cacheData.forEach((row, idx) => {
    ...
    console.log('idx: ', idx)
    console.log('Labels: ', row.l1_grouping, row.l2_grouping, row.l3_grouping)

    // If l1 grouping does not change then don't show it
    if (row.l1_grouping === l1Value) {
      console.log('l1 grouping does not change... erasing value')
    ...
    if (row.l2_grouping === l2Value) {
      console.log('l2 grouping does not change... erasing value')
    ...
    if (row.l2_grouping.toUpperCase().includes('SUBTOTAL')) {
      console.log('l1 is a subtotal, setting values...')

      flattenedMappedData[idx].l1_grouping = `${row.l1_grouping} SUBTOTAL`
      flattenedMappedData[idx].l2_grouping = ''
      flattenedMappedData[idx].l3_grouping = ''

      console.log('was: ', row.l1_grouping, row.l2_grouping, row.l3_grouping)
      console.log('now: ', flattenedMappedData[idx].l1_grouping, flattenedMappedData[idx].l2_grouping, flattenedMappedData[idx].l3_grouping)
    }

    // If l3 grouping includes subtotal then update the labels
    if (row.l3_grouping.toUpperCase().includes('SUBTOTAL')) {
      console.log('l2 is a subtotal, setting values...')
    ...
    if (row.l1_grouping !== l1Value) {
      console.log('l1 grouping does change... setting new value to ', row.l1_grouping)
    ...
    if (row.l2_grouping !== l2Value) {
      console.log('l2 grouping does change... setting new value to ', row.l2_grouping)
